...
filteredMovements0 <- movements0 %>%
  ...
  filter(forwardDistance < 75 | backwardDistance < 75)
```
...
```{r}
coordinates(clusterCoordinates) <- ~longitude+latitude
leaflet(clusterCoordinates) %>% addMarkers(label = samples) %>% addTiles()
```
